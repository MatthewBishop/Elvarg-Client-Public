...
public class ClanSettings {
	...
	void method2950(int var1) {
		...
		if (this.memberRanks != null) {
			System.arraycopy(this.memberRanks, 0, this.memberRanks = new byte[var1], 0, this.memberCount); // L:
																											// 55
		} else {
	...
	void method2974(int var1) {
		if (this.useHashes) { // L: 66
			if (this.bannedMemberHashes != null) {
				System.arraycopy(this.bannedMemberHashes, 0, this.bannedMemberHashes = new long[var1], 0, this.bannedMemberCount); // L:
																																	// 67
			} else {
	...
	void method2956(long var1, String var3, int var4) {
		...
		if (this.useHashes != var1 > 0L) {
			...
			if (var1 > 0L && (this.memberHashes == null || this.memberCount >= this.memberHashes.length) || var3 != null && (this.memberNames == null || this.memberCount >= this.memberNames.length)) { // L:
				// 114
	...
	void method2957(int var1) {
		if (var1 >= 0 && var1 < this.memberCount) {
			...
			if (this.memberCount == 0) {
				...
				System.arraycopy(this.memberRanks, var1 + 1, this.memberRanks, var1, this.memberCount - var1); // L:
																												// 146
				System.arraycopy(this.field1640, var1 + 1, this.field1640, var1, this.memberCount - var1); // L:
																											// 147
	...
	boolean method3016(int var1) {
		if (this.currentOwner != var1 && this.memberRanks[var1] != 126) { // L:
																			// 212
	...
	void method2969(Buffer var1) {
		...
		if (var2 >= 1 && var2 <= 6) {
			...
			if (var2 >= 3) {
				...
				if (var4 > 0) {
					this.parameters = new IterableNodeHashTable(var4 < 16 ? Equations.farthestBitValue(var4) : 16);
					while (var4-- > 0) {
						...
						if (var7 == 0) {
							...
							this.parameters.put(new IntegerNode(var12), (long) var6); // L:
																						// 387
						} else if (var7 == 1) {
							long var10 = var1.readLong(); // L: 390
							this.parameters.put(new class407(var10), (long) var6); // L:
																					// 391
						} else if (var7 == 2) { // L: 393
							String var8 = var1.readStringCp1252NullTerminated();
							this.parameters.put(new ObjectNode(var8), (long) var6); // L:
																					// 395
						}
					}
				}
	...
	static void method3020(AbstractArchive var0, AbstractArchive var1, boolean var2, int var3) {
		if (Login.clearLoginScreen) { // L: 183
			...
			Login.rightTitleSprite = class16.leftTitleSprite.mirrorHorizontally(); // L:
																					// 198
			_Junk.method739(var1, Client.worldProperties);
			...
			Login.field914 = class421.SpriteBuffer_getIndexedSpriteByName(var1, "titlebutton_large", ""); // L:
																											// 202
			...
			Static.field1427 = class421.SpriteBuffer_getIndexedSpriteByName(var1, "options_radio_buttons,4", "");
			class228.options_buttons_2Sprite = class421.SpriteBuffer_getIndexedSpriteByName(var1, "options_radio_buttons,2", "");
			DynamicObject.field979 = class421.SpriteBuffer_getIndexedSpriteByName(var1, "options_radio_buttons,6", ""); // L:
			// 222
	...
	static final void updatePlayers(PacketBuffer var0, int var1) {
		...
		if (var3 != 0) { // L: 104
			...
			if (var3 != 0) { // L: 136
				...
				for (var4 = 0; var4 < Players.Players_emptyIdxCount; ++var4) { // L:
																				// 140
					...
					if ((Players.field1311[var20] & 1) != 0) { // L: 142
						if (var3 > 0) { // L: 143
							...
							if (var6 == 0) { // L: 149
								...
								var10000[var20] = (byte) (var10000[var20] | 2);
							} else if (ModelData0.updateExternalPlayer(var0, var20)) { // L:
																						// 164
					...
					if (var3 != 0) { // L: 200
						...
						for (var4 = 1; var4 < 2048; ++var4) { // L: 205
							...
							if (var5 != null) { // L: 208
								...
								Players.Players_emptyIndices[++Players.Players_emptyIdxCount - 1] = var4; // L:
																											// 209
							}
						}
						for (var3 = 0; var3 < Players.Players_pendingUpdateCount; ++var3) { // L:
																							// 213
							var4 = Players.Players_pendingUpdateIndices[var3]; // L:
																				// 214
							...
							if ((var6 & 4) != 0) { // L: 223
								...
								PlayerType var9 = (PlayerType) Actor.findEnumerated(_Junk.PlayerType_values(), var0.method7766()); // L:
																																			// 225
								boolean var10 = var0.method7766() == 1; // L:
																		// 226
								var11 = var0.readUnsignedByte(); // L: 227
								var12 = var0.offset; // L: 228
								if (var5.username != null && var5.appearance != null) { // L:
																						// 229
									boolean var13 = false; // L: 230
									if (var9.isUser && Player.friendSystem.isIgnored(var5.username)) { // L:
																										// 231
																										// 232
										var13 = true;
									}
									if (!var13 && Client.field607 == 0 && !var5.isHidden) { // L:
																							// 234
										Players.field1317.offset = 0; // L: 235
										var0.method7828(Players.field1317.array, 0, var11); // L:
																							// 236
										Players.field1317.offset = 0; // L: 237
										String var14 = AbstractFont.escapeBrackets(ArchiveDiskAction.method5777(class134.method2904(Players.field1317))); // L:
										// 238
										var5.overheadText = var14.trim(); // L:
																			// 239
										var5.overheadTextColor = var8 >> 8; // L:
																			// 240
										var5.overheadTextEffect = var8 & 255; // L:
																				// 241
										var5.overheadTextCyclesRemaining = 150; // L:
																				// 242
										var5.isAutoChatting = var10; // L: 243
										var5.field1152 = var5 != class101.localPlayer && var9.isUser && "" != Client.field716 && var14.toLowerCase().indexOf(Client.field716) == -1; // L:
																																														// 244
										...
										if (var9.modIcon != -1) { // L: 248
											...
											class290.addGameMessage(var15, var5.username.getName(), var14); // L:
																											// 249
										}
									}
								}
							...
							if ((var6 & 8192) != 0) { // L: 254
								var5.spotAnimation = var0.method7776(); // L:
																		// 255
								var8 = var0.readInt(); // L: 256
								var5.spotAnimationHeight = var8 >> 16; // L: 257
								var5.field1178 = (var8 & 65535) + Client.cycle; // L:
																				// 258
							...
							if ((var6 & 1024) != 0) { // L: 264
								...
								var5.field1184 = var0.method7774() + Client.cycle; // L:
																					// 269
								var5.field1194 = var0.method7776() + Client.cycle; // L:
																					// 270
								var5.field1186 = var0.readUnsignedShort(); // L:
																			// 271
							...
							if ((var6 & 256) != 0) { // L: 295
								...
								class192[] var18 = new class192[]{class192.field2212, class192.field2208, class192.field2209, class192.field2211}; // L:
																																					// 300
								var16[var4] = (class192) Actor.findEnumerated(var18, var0.method7952()); // L:
																											// 302
							}
							if ((var6 & 1) != 0) { // L: 304
								var5.overheadText = var0.readStringCp1252NullTerminated(); // L:
																							// 305
								if (var5.overheadText.charAt(0) == '~') { // L:
																			// 306
									var5.overheadText = var5.overheadText.substring(1); // L:
																						// 307
									class290.addGameMessage(2, var5.username.getName(), var5.overheadText); // L:
																											// 308
								} else if (var5 == class101.localPlayer) { // L:
																			// 310
									class290.addGameMessage(2, var5.username.getName(), var5.overheadText); // L:
																											// 311
								}
							...
							if ((var6 & 2) != 0) { // L: 318
								...
								class65.performPlayerAnimation(var5, var8, var17); // L:
																					// 322
							}
							if ((var6 & 32) != 0) { // L: 324
								...
								int var24;
								if (var8 > 0) { // L: 326
									for (var17 = 0; var17 < var8; ++var17) { // L:
																				// 327
										var11 = -1; // L: 329
										var12 = -1; // L: 330
										var24 = -1; // L: 331
										var23 = var0.readUShortSmart(); // L:
																		// 332
										if (var23 == 32767) { // L: 333
											var23 = var0.readUShortSmart(); // L:
																			// 334
											var12 = var0.readUShortSmart(); // L:
																			// 335
											var11 = var0.readUShortSmart(); // L:
																			// 336
											var24 = var0.readUShortSmart(); // L:
																			// 337
										...
										var19 = var0.readUShortSmart(); // L:
																		// 343
										var5.addHitSplat(var23, var12, var11, var24, Client.cycle, var19); // L:
																											// 344
									}
								}
								var17 = var0.method7767(); // L: 347
								if (var17 > 0) { // L: 348
									for (var23 = 0; var23 < var17; ++var23) { // L:
																				// 349
										var11 = var0.readUShortSmart(); // L:
																		// 350
										var12 = var0.readUShortSmart(); // L:
										...
										if (var12 != 32767) { // L: 352
											...
											var15 = var12 > 0 ? var0.method7766() : var19; // L:
																							// 355
											var5.addHealthBar(var11, Client.cycle, var12, var24, var19, var15); // L:
											// 356
										} else {
											var5.removeHealthBar(var11); // L:
																			// 358
										}
									}
								}
							...
							if ((var6 & 512) != 0) { // L: 366
								var5.field1189 = Client.cycle + var0.method7774(); // L:
																					// 367
								var5.field1135 = Client.cycle + var0.method7776(); // L:
																					// 368
								var5.field1191 = var0.method7769(); // L: 369
								var5.field1192 = var0.method7952(); // L: 370
								var5.field1193 = var0.method7769(); // L: 371
								var5.field1133 = (byte) var0.readUnsignedByte(); // L:
																					// 372
							}
							...
							if (var5.field1088) { // L: 385
								if (var7 == 127) { // L: 386
									...
									class192 var25;
									if (var7 != class192.field2208.field2210) { // L:
																				// 389
										class192[] var22 = new class192[]{class192.field2212, class192.field2208, class192.field2209, class192.field2211}; // L:
																																							// 392
										var25 = (class192) Actor.findEnumerated(var22, var7); // L:
																								// 394
									} else {
										var25 = Players.field1304[var4]; // L:
																			// 396
									}
									var5.method2139(var5.tileX, var5.tileY, var25); // L:
																					// 397
								}
							}
						}
						if (var0.offset - var2 != var1) { // L: 404
							throw new RuntimeException(var0.offset - var2 + " " + var1); // L:
																							// 405
						}
					}
				}
