...
public class FriendSystem {
	...
	final void readUpdate(Buffer var1, int var2) {
		...
		for (int var3 = 0; var3 < Players.Players_count; ++var3) { // L: 43
			Player var4 = Client.players[Players.Players_indices[var3]]; // L:
																			// 44
	...
	final void processFriendUpdates() {
		for (FriendLoginUpdate var1 = (FriendLoginUpdate) this.friendsList.friendLoginUpdates.last(); var1 != null; var1 = (FriendLoginUpdate) this.friendsList.friendLoginUpdates.previous()) { // L:
			// 62
			if ((long) var1.field4300 < Equations.getCurrentTime() / 1000L - 5L) { // L:
																				// 63
				if (var1.world > 0) {
					class290.addGameMessage(5, "", var1.username + " has logged in."); // L:
																						// 64
				}
	...
	final void addFriend(String var1) {
		if (var1 != null) { // L: 91
			...
			if (var2.hasCleanName()) { // L: 93
				...
				if (this.friendsListIsFull()) { // L: 94
				...
				} else if (class101.localPlayer.username.equals(var2)) { // L:
																			// 103
					var10000 = null; // L: 105
					...
					PacketBufferNode var3 = EnumComposition.getPacketBufferNode(ClientPacket.field2964, Client.packetWriter.isaacCipher); // L:
																																			// 127
					var3.packetBuffer.writeByte(ScriptEvent.stringCp1252NullTerminatedByteSize(var1)); // L:
																										// 128
					var3.packetBuffer.writeStringCp1252NullTerminated(var1); // L:
																				// 129
	...
	final boolean friendsListIsFull() {
		return this.friendsList.isFull() || this.friendsList.getSize() >= 200 && Client.field601 != 1; // L:
																										// 142
	}
				...
				if (this.canAddIgnore()) { // L: 152
					...
					if (class101.localPlayer.username.equals(var2)) { // L: 156
						...
						if (this.isIgnored(var2)) { // L: 165
							var10000 = (new StringBuilder()).append(var1); // L:
																			// 167
							var10001 = null;
	...
	final boolean canAddIgnore() {
		return this.ignoreList.isFull() || this.ignoreList.getSize() >= 100 && Client.field601 != 1; // L:
																										// 202
	}
		...
		if (var1 != null) { // L: 209
			...
			if (var2.hasCleanName()) { // L: 211
				if (this.friendsList.removeByUsername(var2)) { // L: 212
					...
					PacketBufferNode var3 = EnumComposition.getPacketBufferNode(ClientPacket.field2958, Client.packetWriter.isaacCipher); // L:
																																			// 217
					var3.packetBuffer.writeByte(ScriptEvent.stringCp1252NullTerminatedByteSize(var1)); // L:
																										// 218
					var3.packetBuffer.writeStringCp1252NullTerminated(var1); // L:
				...
				for (int var5 = 0; var5 < Players.Players_count; ++var5) { // L:
																			// 223
					Player var4 = Client.players[Players.Players_indices[var5]]; // L:
																					// 224
					var4.clearIsFriend(); // L: 225
				}
				Iterator var6 = Messages.Messages_hashTable.iterator(); // L:
																		// 228
				while (var6.hasNext()) {
		...
		if (var1 != null) { // L: 242
			...
			if (var2.hasCleanName()) { // L: 244
				if (this.ignoreList.removeByUsername(var2)) { // L: 245
					...
					PacketBufferNode var3 = EnumComposition.getPacketBufferNode(ClientPacket.field2921, Client.packetWriter.isaacCipher); // L:
																																			// 250
					var3.packetBuffer.writeByte(ScriptEvent.stringCp1252NullTerminatedByteSize(var1)); // L:
																										// 251
					var3.packetBuffer.writeStringCp1252NullTerminated(var1); // L:
