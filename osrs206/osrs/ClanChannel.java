...
public class ClanChannel extends Node {
	...
	public int[] getSortedMembers() {
		if (this.sortedMembers == null) { // L: 30
			...
			for (int var2 = 0; var2 < this.members.size(); this.sortedMembers[var2] = var2++) { // L:
																								// 33
																								// 35
				var1[var2] = ((ClanChannelMember) this.members.get(var2)).username.method8320(); // L:
																									// 34
			}
	...
	static void requestNetFile(Archive var0, int var1, int var2, int var3, byte var4, boolean var5) {
		...
		NetFileRequest var8 = (NetFileRequest) NetCache.NetCache_pendingPriorityWrites.get(var6); // L:
																									// 102
		if (var8 == null) { // L: 103
			var8 = (NetFileRequest) NetCache.NetCache_pendingPriorityResponses.get(var6); // L:
																							// 104
			if (var8 == null) { // L: 105
				var8 = (NetFileRequest) NetCache.NetCache_pendingWrites.get(var6); // L:
																					// 106
				if (var8 != null) { // L: 107
					if (var5) { // L: 108
						var8.removeDual(); // L: 109
						NetCache.NetCache_pendingPriorityWrites.put(var8, var6); // L:
																					// 110
					...
					if (!var5) { // L: 116
						var8 = (NetFileRequest) NetCache.NetCache_pendingResponses.get(var6); // L:
																								// 117
						if (var8 != null) { // L: 118
							return;
						}
					...
					if (var5) { // L: 124
						...
						NetCache.NetCache_pendingWritesQueue.addFirst(var8); // L:
																				// 129
						NetCache.NetCache_pendingWrites.put(var8, var6); // L:
																			// 130
