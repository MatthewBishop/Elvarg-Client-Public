...
public class ArchiveDiskAction extends Node {
	...
	public static void performReflectionCheck(PacketBuffer var0) {
		ReflectionCheck var1 = (ReflectionCheck) class33.reflectionChecks.last(); // L:
																					// 35
		if (var1 != null) { // L: 36
			...
			for (int var3 = 0; var3 < var1.size; ++var3) { // L: 39
				if (var1.creationErrors[var3] != 0) { // L: 40
					...
					try {
						...
						//	var6 = Reflection.getInt(var5, (Object) null); // L:
																			// 48
							var0.writeByte(0); // L: 49
						//	var0.writeInt(var6); // L: 50
						} else if (var4 == 1) { // L: 52
							var5 = var1.fields[var3]; // L: 53
						//	Reflection.setInt(var5, (Object) null, var1.intReplaceValues[var3]); // L:
																									// 54
						...
						if (var4 != 3) { // L: 63
							...
							for (int var8 = 0; var8 < var10.length; ++var8) { // L:
																				// 67
								ObjectInputStream var9 = new ObjectInputStream(new ByteArrayInputStream(var10[var8])); // L:
								// 68
								var7[var8] = var9.readObject(); // L: 69
							}
					/*		Object var11 = Reflection.invoke(var25, (Object) null, var7); // L:
																							// 71
							if (var11 == null) { // L: 72
								var0.writeByte(0);
							} else if (var11 instanceof Number) { // L: 73
								var0.writeByte(1); // L: 74
								var0.writeLong(((Number) var11).longValue()); // L:
																				// 75
							} else if (var11 instanceof String) { // L: 77
								var0.writeByte(2); // L: 78
								var0.writeStringCp1252NullTerminated((String) var11); // L:
																						// 79
							} else {
