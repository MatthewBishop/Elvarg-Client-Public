...
public class BufferedSource implements Runnable {
			...
			synchronized (this) { // L: 156
				...
				if (var3 == 0 && this.exception != null) { // L: 161
					...
					if (var3 + this.position <= this.capacity) { // L: 162
						System.arraycopy(this.buffer, this.position, var1, var2, var3); // L:
																						// 163
					} else {
						int var6 = this.capacity - this.position; // L: 166
						System.arraycopy(this.buffer, this.position, var1, var2, var6); // L:
																						// 167
						System.arraycopy(this.buffer, 0, var1, var6 + var2, var3 - var6); // L:
																							// 168
					}
					this.position = (var3 + this.position) % this.capacity; // L:
																			// 170
	...
	public void run() {
		while (true) {
			...
			int var7;
			try {
				var7 = this.inputStream.read(this.buffer, this.limit, var1); // L:
																				// 97
				if (var7 == -1) {
