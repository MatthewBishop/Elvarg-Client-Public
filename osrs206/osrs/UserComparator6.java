...
public class UserComparator6 extends AbstractUserComparator {
	...
	public static class435 method2626(int var0) {
		class435 var1 = (class435) class435.DBTableType_cache.get((long) var0); // L:
																				// 23
		if (var1 != null) { // L: 24
			return var1;
		} else {
	...
	static int method2628(int var0, Script var1, boolean var2) {
		...
		if (var0 == ScriptOpcodes.OC_NAME) { // L: 3438
			var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																							// 3439
			Interpreter.Interpreter_stringStack[++Interpreter.Interpreter_stringStackSize - 1] = EnumComposition.ItemDefinition_get(var3).name; // L:
																																				// 3440
			return 1; // L: 3441
		} else {
			int var4;
			...
			if (var0 == ScriptOpcodes.OC_OP) { // L: 3443
				...
				if (var4 >= 1 && var4 <= 5 && var5.groundActions[var4 - 1] != null) {
					Interpreter.Interpreter_stringStack[++Interpreter.Interpreter_stringStackSize - 1] = var5.groundActions[var4 - 1]; // L:
																																		// 3448
				} else {
				...
				if (var4 >= 1 && var4 <= 5 && var5.inventoryActions[var4 - 1] != null) { // L:
																							// 3457
				...
				var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																								// 3462
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = EnumComposition.ItemDefinition_get(var3).price; // L:
																																			// 3463
				return 1; // L: 3464
			} else if (var0 == ScriptOpcodes.OC_STACKABLE) { // L: 3466
				var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																								// 3467
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = EnumComposition.ItemDefinition_get(var3).isStackable == 1 ? 1 : 0; // L:
																																								// 3468
				return 1; // L: 3469
			} else {
				...
				if (var0 == ScriptOpcodes.OC_CERT) { // L: 3471
					...
					var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																									// 3486
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = EnumComposition.ItemDefinition_get(var3).isMembersOnly ? 1 : 0; // L:
																																								// 3487
					return 1; // L: 3488
					...
					if (var7.placeholderTemplate == -1 && var7.placeholder >= 0) { // L:
																					// 3493
					...
					if (var7.placeholderTemplate >= 0 && var7.placeholder >= 0) { // L:
																					// 3500
						Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var7.placeholder;
					} else {
						Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var3; // L:
					...
					String var6 = Interpreter.Interpreter_stringStack[--Interpreter.Interpreter_stringStackSize]; // L:
					// 3505
					var4 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																									// 3506
					_Junk.findItemDefinitions(var6, var4 == 1); // L:
																					// 3507
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = HealthBarUpdate.foundItemIdCount; // L:
																																	// 3508
					return 1; // L: 3509
					...
					if (var0 == ScriptOpcodes.OC_FINDRESET) { // L: 3516
						...
						var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																										// 3521
						var4 = EnumComposition.ItemDefinition_get(var3).getShiftClickIndex(); // L:
																								// 3522
						if (var4 == -1) { // L: 3523
							Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var4; // L:
																												// 3524
						} else {
							Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var4 + 1; // L:
							// 3527
						}
						return 1; // L: 3529
					} else {
					...
					if (class10.foundItemIds != null && TriBool.foundItemIndex < HealthBarUpdate.foundItemIdCount) { // L:
						// 3512
						Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = class10.foundItemIds[++TriBool.foundItemIndex - 1] & '\uffff'; // L:
																																									// 3513
					} else {
