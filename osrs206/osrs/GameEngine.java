...
public abstract class GameEngine extends Applet implements Runnable, FocusListener, WindowListener {
	...
	protected GameEngine() {
		...
		StaticSound.init(new _AudioBridge());
	} // L: 96
	
	protected final void setMaxCanvasSize(int var1, int var2) {
		if (this.maxCanvasWidth != var1 || var2 != this.maxCanvasHeight) { // L:
																			// 99
			this.method477();
		}
	...
	final void post(Object var1) {
		if (this.eventQueue != null) { // L: 105
			for (int var2 = 0; var2 < 50 && this.eventQueue.peekEvent() != null; ++var2) { // L:
																							// 106
				DynamicObject.method1991(1L);
			}
			if (var1 != null) {
				this.eventQueue.postEvent(new ActionEvent(var1, 1001, "dummy")); // L:
																					// 107
			}
		}
	} // L: 108
	...
	protected void method385(String var1) {
		this.clipboard.setContents(new StringSelection(var1), (ClipboardOwner) null); // L:
																						// 123
	} // L: 124
	...
	protected final void method387() {
		...
		var1.addMouseMotionListener(MouseHandler.MouseHandler_instance); // L:
																			// 135
	...
	final void resizeCanvas() {
		...
		if (var1 != null) { // L: 142
			...
			Script.canvasWidth = Math.min(this.contentWidth, this.maxCanvasWidth); // L:
																					// 148
			class78.canvasHeight = Math.min(this.contentHeight, this.maxCanvasHeight); // L:
																						// 149
			this.canvasX = (this.contentWidth - Script.canvasWidth) / 2; // L:
																			// 150
			this.canvasY = 0; // L: 151
			this.canvas.setSize(Script.canvasWidth, class78.canvasHeight); // L:
																			// 152
			Message.rasterProvider = new RasterProvider(Script.canvasWidth, class78.canvasHeight, this.canvas); // L:
			// 153
			if (var1 == this.frame) { // L: 154
				Insets var3 = this.frame.getInsets(); // L: 155
				this.canvas.setLocation(this.canvasX + var3.left, var3.top + this.canvasY); // L:
																							// 156
			} else {
	...
	void clearBackground() {
		...
		if (var1 > 0 || var3 > 0 || var2 > 0 || var4 > 0) { // L: 170
			try {
				...
				if (var1 > 0) {
					var10.fillRect(var6, var7, var1, this.contentHeight); // L:
																			// 182
				}
				if (var2 > 0) { // L: 183
	...
	final void replaceCanvas() {
		...
		var1.removeMouseMotionListener(MouseHandler.MouseHandler_instance); // L:
																			// 196
		...
		var2.addMouseMotionListener(MouseHandler.MouseHandler_instance); // L:
																			// 206
	...
	final synchronized void addCanvas() {
		...
		class78.canvasHeight = Math.max(var1.getHeight(), this.field193); // L:
																			// 246
		Insets var2;
		...
		if (var1 == this.frame) { // L: 259
			...
			this.canvas.setLocation(var2.left + this.canvasX, this.canvasY + var2.top); // L:
																						// 261
		} else {
		...
		if (Message.rasterProvider != null && Script.canvasWidth == Message.rasterProvider.width && class78.canvasHeight == Message.rasterProvider.height) { // L:
																																								// 267
			((RasterProvider) Message.rasterProvider).setComponent(this.canvas); // L:
																					// 268
		...
		this.field185 = Equations.getCurrentTime(); // L: 275
	} // L: 276
	
	protected final boolean checkHost() {
		String var1 = this.getDocumentBase().getHost().toLowerCase(); // L: 279
		if (!var1.equals("jagex.com") && !var1.endsWith(".jagex.com")) { // L:
																			// 280
			if (!var1.equals("runescape.com") && !var1.endsWith(".runescape.com")) { // L:
																						// 281
	...
	void clientTick() {
		long var1 = Equations.getCurrentTime(); // L: 330
	...
	void graphicsTick() {
		...
		long var2 = Equations.getCurrentTime(); // L: 343
		...
		if (++field199 - 1 > 50) { // L: 351
			...
			if (var1 == this.frame) { // L: 356
				...
				this.canvas.setLocation(var7.left + this.canvasX, this.canvasY + var7.top); // L:
																							// 358
			} else {
	...
	final void method397() {
		...
		if (this.contentWidth != var1.highX || var1.highY != this.contentHeight || this.resizeCanvasNextFrame) { // L:
			// 371
	...
	protected final void drawInitial(int var1, String var2, boolean var3) {
		try {
			...
			if (Static.fontHelvetica13 == null) { // L: 492
				Static.fontHelvetica13 = new java.awt.Font("Helvetica", 1, 13); // L:
																					// 493
				class11.loginScreenFontMetrics = this.canvas.getFontMetrics(Static.fontHelvetica13); // L:
																										// 494
			}
			if (var3) { // L: 496
				var4.setColor(Color.black); // L: 497
				var4.fillRect(0, 0, Script.canvasWidth, class78.canvasHeight); // L:
																				// 498
			}
			...
			try {
				...
				var6.setFont(Static.fontHelvetica13); // L: 510
				var6.setColor(Color.white); // L: 511
				var6.drawString(var2, (304 - class11.loginScreenFontMetrics.stringWidth(var2)) / 2, 22); // L:
																											// 512
				var4.drawImage(class162.field1781, Script.canvasWidth / 2 - 152, class78.canvasHeight / 2 - 18, (ImageObserver) null); // L:
																																		// 513
				...
				var4.fillRect(var1 * 3 + var7 + 2, var8 + 2, 300 - var1 * 3, 30); // L:
																					// 523
				var4.setFont(Static.fontHelvetica13); // L: 524
				var4.setColor(Color.white); // L: 525
				var4.drawString(var2, var7 + (304 - class11.loginScreenFontMetrics.stringWidth(var2)) / 2, var8 + 22); // L:
				// 526
			}
	...
	protected final void method410() {
		...
		Static.fontHelvetica13 = null; // L: 536
	...
	protected void error(String var1) {
		if (!this.hasErrored) { // L: 541
			...
			try {
				this.getAppletContext().showDocument(new URL(this.getCodeBase(), "error_game_" + var1 + ".ws"), "_self"); // L:
																															// 545
	...
	public final void destroy() {
		if (this == gameEngine && !isKilled) { // L: 428
			stopTimeMs = Equations.getCurrentTime(); // L: 429
	...
	public final synchronized void paint(Graphics var1) {
		if (this == gameEngine && !isKilled) { // L: 441
			...
			if (Equations.getCurrentTime() - this.field185 > 1000L) { // L: 443
	...
	public void run() {
		try {
			if (class360.javaVendor != null) { // L: 291
				...
				if (var1.indexOf("sun") != -1 || var1.indexOf("apple") != -1) { // L:
																				// 293
					String var2 = TaskHandler.javaVersion; // L: 294
					if (var2.equals("1.1") || var2.startsWith("1.1.") || var2.equals("1.2") || var2.startsWith("1.2.") || var2.equals("1.3") || var2.startsWith("1.3.") || var2.equals("1.4") || var2.startsWith("1.4.") || var2.equals("1.5") || var2.startsWith("1.5.") || var2.equals("1.6.0")) { // L:
																																																																										// 295
					...
					if (var2.startsWith("1.6.0_")) { // L: 299
						int var3;
						for (var3 = 6; var3 < var2.length() && class83.isDigit(var2.charAt(var3)); ++var3) { // L:
																												// 300
							// 301
						}
						String var4 = var2.substring(6, var3); // L: 302
						if (class114.isNumber(var4) && FaceNormal.method4344(var4) < 10) { // L:
																							// 303
			...
			while (0L == stopTimeMs || Equations.getCurrentTime() < stopTimeMs) { // L:
																				// 315
				Messages.gameCyclesToDo = class10.clock.wait(cycleDurationMillis, fiveOrOne); // L:
																								// 316
				for (int var5 = 0; var5 < Messages.gameCyclesToDo; ++var5) { // L:
																				// 317
					this.clientTick();
				}
	...
	public final void stop() {
		if (this == gameEngine && !isKilled) { // L: 422
			stopTimeMs = Equations.getCurrentTime() + 4000L; // L: 423
		}
	} // L: 424
	...
	static File getFile(String var0) {
		if (!FileSystem.FileSystem_hasPermissions) { // L: 22
			...
			File var1 = (File) FileSystem.FileSystem_cacheFiles.get(var0); // L:
																			// 23
			if (var1 != null) { // L: 24
				return var1;
			} else {
				File var2 = new File(FileSystem.FileSystem_cacheDir, var0); // L:
																			// 25
				...
				try {
					...
					if (!var4.exists()) { // L: 29
						...
						var3.seek(0L); // L: 34
						var3.close(); // L: 35
						FileSystem.FileSystem_cacheFiles.put(var0, var2); // L:
																			// 36
						return var2; // L: 37
					}
