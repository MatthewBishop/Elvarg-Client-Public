...
public class FileSystem {
	...
	public static void method3282() {
		SpotAnimationDefinition.SpotAnimationDefinition_cached.clear(); // L:
																		// 117
		SpotAnimationDefinition.SpotAnimationDefinition_cachedModels.clear(); // L:
																				// 118
	} // L: 119
	
	protected static int getGcDuration() {
		int var0 = 0; // L: 572
		if (class162.garbageCollector == null || !class162.garbageCollector.isValid()) { // L:
																							// 573
			try {
				Iterator var1 = ManagementFactory.getGarbageCollectorMXBeans().iterator(); // L:
																							// 575
				while (var1.hasNext()) {
					GarbageCollectorMXBean var2 = (GarbageCollectorMXBean) var1.next(); // L:
																						// 576
					if (var2.isValid()) { // L: 578
						class162.garbageCollector = var2; // L: 579
						GameEngine.garbageCollectorLastCheckTimeMs = -1L; // L:
																			// 580
						GameEngine.garbageCollectorLastCollectionTime = -1L; // L:
																				// 581
					}
				}
		...
		if (class162.garbageCollector != null) { // L: 588
			long var9 = Equations.getCurrentTime(); // L: 589
			long var3 = class162.garbageCollector.getCollectionTime(); // L: 590
			if (GameEngine.garbageCollectorLastCollectionTime != -1L) { // L:
																		// 591
				long var5 = var3 - GameEngine.garbageCollectorLastCollectionTime; // L:
																					// 592
				long var7 = var9 - GameEngine.garbageCollectorLastCheckTimeMs; // L:
																				// 593
				if (0L != var7) { // L: 594
