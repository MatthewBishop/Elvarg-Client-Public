...
public class UserComparator10 extends AbstractUserComparator {
	...
	public static class167 method2609(int var0, Buffer var1) {
		...
		if (var3) { // L: 28
			...
			var8 = var5.field1814 != null && var7.length == var5.field1814.length; // L:
																					// 33
		...
		if (var4) { // L: 43
			...
			var8 = var5.field1815 != null && var7.length == var5.field1815.length; // L:
																					// 48
		...
		label68 : while (true) {
			...
			while (true) {
				...
				while (!var8) { // L: 198
					...
					if (var13 > 0 && var14 > 0 && var13 < 103 && var14 < 103) { // L:
																				// 212
						ObjectComposition var15 = class162.getObjectDefinition(var5); // L:
																						// 213
						if (var12 != 22 || !Client.isLowDetail || var15.int1 != 0 || var15.interactType == 1 || var15.boolean2) { // L:
																																	// 214
	...
	static int method2605(int var0, Script var1, boolean var2) {
		if (var0 == ScriptOpcodes.CLIENTCLOCK) { // L: 2134
			Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.cycle; // L:
																										// 2135
			return 1; // L: 2136
		} else {
			int var3;
			...
			if (var0 == ScriptOpcodes.INV_GETOBJ) { // L: 2138
				...
				var4 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]; // L:
																								// 2141
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = class152.method3147(var3, var4); // L:
																																// 2142
				return 1; // L: 2143
				...
				var4 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]; // L:
																								// 2148
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Archive.ItemContainer_getCount(var3, var4); // L:
																																		// 2149
				return 1; // L: 2150
				...
				var4 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]; // L:
																								// 2155
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Bounds.method6922(var3, var4); // L:
																															// 2156
				return 1; // L: 2157
			} else if (var0 == ScriptOpcodes.INV_SIZE) { // L: 2159
				var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																								// 2160
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Calendar.getInvDefinition(var3).size; // L:
																																	// 2161
				return 1; // L: 2162
			} else if (var0 == ScriptOpcodes.STAT) { // L: 2164
				var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																								// 2165
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.currentLevels[var3]; // L:
				// 2166
				return 1; // L: 2167
			} else if (var0 == ScriptOpcodes.STAT_BASE) { // L: 2169
				var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																								// 2170
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.levels[var3]; // L:
				// 2171
				return 1; // L: 2172
			} else if (var0 == ScriptOpcodes.STAT_XP) { // L: 2174
				var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																								// 2175
				Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.experience[var3]; // L:
				// 2176
				return 1; // L: 2177
			} else {
				int var5;
				if (var0 == ScriptOpcodes.COORD) { // L: 2179
					...
					var4 = (class101.localPlayer.x >> 7) + class28.baseX; // L:
																			// 2181
					var5 = (class101.localPlayer.y >> 7) + WorldMapLabelSize.baseY; // L:
																					// 2182
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = (var4 << 14) + var5 + (var3 << 28); // L:
																																	// 2183
					return 1; // L: 2184
				} else if (var0 == ScriptOpcodes.COORDX) { // L: 2186
					var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																									// 2187
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var3 >> 14 & 16383; // L:
					// 2188
					return 1; // L: 2189
				} else if (var0 == ScriptOpcodes.COORDZ) { // L: 2191
					var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																									// 2192
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var3 >> 28; // L:
																											// 2193
					return 1; // L: 2194
				} else if (var0 == ScriptOpcodes.COORDY) { // L: 2196
					var3 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																									// 2197
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = var3 & 16383; // L:
																												// 2198
					return 1; // L: 2199
				} else if (var0 == ScriptOpcodes.MAP_MEMBERS) { // L: 2201
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.isMembersWorld ? 1 : 0; // L:
																																// 2202
					return 1; // L: 2203
				} else if (var0 == ScriptOpcodes.INVOTHER_GETOBJ) { // L: 2205
					class446.Interpreter_intStackSize -= 2; // L: 2206
					var3 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize] + 32768; // L:
																										// 2207
					var4 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]; // L:
																									// 2208
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = class152.method3147(var3, var4); // L:
					...
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.rebootTimer; // L:
					// 2232
					return 1; // L: 2233
				} else if (var0 == ScriptOpcodes.MAP_WORLD) { // L: 2235
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.worldId; // L:
					// 2236
					return 1; // L: 2237
				} else if (var0 == ScriptOpcodes.RUNENERGY_VISIBLE) { // L: 2239
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.runEnergy; // L:
					// 2240
					return 1; // L: 2241
				} else if (var0 == ScriptOpcodes.RUNWEIGHT_VISIBLE) { // L: 2243
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.weight; // L:
					// 2244
					return 1; // L: 2245
					...
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.worldProperties; // L:
					// 2253
					return 1; // L: 2254
					...
					var5 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 2]; // L:
																									// 2260
					int var6 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 3]; // L:
																										// 2261
					var3 += var4 << 14; // L: 2262
					var3 += var5 << 28; // L: 2263
					var3 += var6; // L: 2264
					...
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.field605; // L:
					// 2269
					return 1; // L: 2270
				} else if (var0 == 3327) { // L: 2272
					Interpreter.Interpreter_intStack[++class446.Interpreter_intStackSize - 1] = Client.field647; // L:
					// 2273
					return 1; // L: 2274
				} else {
					return 2; // L: 2276
	...
	static final void worldToMinimap(int var0, int var1, int var2, int var3, SpritePixels var4, SpriteMask var5) {
		...
		if (var6 > 4225 && var6 < 90000) { // L: 12164
			...
			double var12 = Math.atan2((double) var10, (double) var11); // L:
																		// 12170
			...
			class344.redHintArrowSprite.method8179(var15 + (var0 + var5.width / 2 - var17 / 2), var5.height / 2 + var1 - var17 / 2 - var16 - 10, var17, var17, 15, 15, var12, 256); // L:
																																													// 12175
		} else {
			ParamComposition.drawSpriteOnMinimap(var0, var1, var2, var3, var4, var5); // L:
																						// 12177
		}
	} // L: 12178
	
	static Widget method2610(Widget var0) {
		int var1 = class134.method2906(class124.getWidgetFlags(var0)); // L:
																		// 12295
		if (var1 == 0) { // L: 12296
			return null;
		} else {
