				...
				if (this.exception != null) { // L: 270
					...
					if (var5 < var3) {
						...
						if (var3 + this.limit <= this.capacity) { // L: 275
							System.arraycopy(var1, var2, this.buffer, this.limit, var3); // L:
																							// 276
						} else {
							int var6 = this.capacity - this.limit; // L: 279
							System.arraycopy(var1, var2, this.buffer, this.limit, var6); // L:
																							// 280
							System.arraycopy(var1, var6 + var2, this.buffer, 0, var3 - var6); // L:
																								// 281
						}
						this.limit = (var3 + this.limit) % this.capacity; // L:
																			// 283
		...
		do {
			...
			synchronized (this) { // L: 224
				while (true) {
					...
					if (this.position <= this.limit) { // L: 227
						...
						var1 = this.capacity - this.position + this.limit; // L:
																			// 228
					}
					if (var1 > 0) { // L: 229
						break;
			...
			try {
				if (var1 + this.position <= this.capacity) { // L: 245
					this.outputStream.write(this.buffer, this.position, var1); // L:
																				// 246
				} else {
					int var7 = this.capacity - this.position; // L: 249
					this.outputStream.write(this.buffer, this.position, var7); // L:
																				// 250
					this.outputStream.write(this.buffer, 0, var1 - var7); // L:
																			// 251
				}
			...
			synchronized (this) { // L: 260
				this.position = (var1 + this.position) % this.capacity; // L:
																		// 261
			} // L: 262
