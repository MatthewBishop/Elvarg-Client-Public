...
public class ChatChannel {
	...
	Message getMessage(int var1) {
		return var1 >= 0 && var1 < this.count ? this.messages[var1] : null; // L:
																			// 102
																			// 103
	}
	...
	static int method2003(int var0, Script var1, boolean var2) {
		...
		if (var0 >= 2000) { // L: 870
			var0 -= 1000; // L: 871
			var4 = class140.getWidget(Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]); // L:
																												// 872
			var3 = false; // L: 873
		} else {
		...
		if (var0 == ScriptOpcodes.CC_SETOP) { // L: 876
			var11 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] - 1; // L:
																								// 877
			if (var11 >= 0 && var11 <= 9) { // L: 878
				var4.setAction(var11, Interpreter.Interpreter_stringStack[--Interpreter.Interpreter_stringStackSize]); // L:
				// 882
				return 1; // L: 883
			} else {
			...
			if (var0 == ScriptOpcodes.CC_SETDRAGGABLE) { // L: 885
				...
				var11 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize]; // L:
																								// 887
				...
				return 1; // L: 890
			} else if (var0 == ScriptOpcodes.CC_SETDRAGGABLEBEHAVIOR) { // L:
																		// 892
				var4.isScrollBar = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L:
																												// 893
				return 1; // L: 894
			} else if (var0 == ScriptOpcodes.CC_SETDRAGDEADZONE) { // L: 896
				var4.dragZoneSize = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																											// 897
				return 1; // L: 898
			} else if (var0 == ScriptOpcodes.CC_SETDRAGDEADTIME) { // L: 900
				var4.dragThreshold = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																											// 901
				return 1; // L: 902
			} else if (var0 == ScriptOpcodes.CC_SETOPBASE) { // L: 904
				var4.dataText = Interpreter.Interpreter_stringStack[--Interpreter.Interpreter_stringStackSize]; // L:
				// 905
				return 1; // L: 906
			} else if (var0 == ScriptOpcodes.CC_SETTARGETVERB) { // L: 908
				var4.spellActionName = Interpreter.Interpreter_stringStack[--Interpreter.Interpreter_stringStackSize]; // L:
				// 909
				return 1; // L: 910
				...
				var4.prioritizeMenuEntry = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] == 1; // L:
				// 917
				return 1; // L: 918
				...
				if (var0 != ScriptOpcodes.CC_SETOPKEY) { // L: 924
					...
					if (var0 == ScriptOpcodes.CC_SETOPTKEY) { // L: 954
						...
						var5 = 10; // L: 956
						var8 = new byte[]{(byte) Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize]}; // L:
																														// 957
						byte[] var9 = new byte[]{(byte) Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]}; // L:
						// 958
						class340.Widget_setKey(var4, var5, var8, var9); // L:
																		// 959
						return 1; // L: 960
					} else if (var0 == ScriptOpcodes.CC_SETOPKEYRATE) { // L:
																		// 962
						...
						var6 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]; // L:
																										// 965
						var7 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 2]; // L:
																										// 966
						if (var11 >= 0 && var11 <= 9) { // L: 967
							class346.Widget_setKeyRate(var4, var11, var6, var7); // L:
																					// 970
							return 1; // L: 971
						} else {
							throw new RuntimeException(); // L: 968
						}
					} else if (var0 == ScriptOpcodes.CC_SETOPTKEYRATE) { // L:
																			// 973
						var5 = 10; // L: 974
						...
						var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize]; // L:
																										// 976
						class346.Widget_setKeyRate(var4, var5, var6, var7); // L:
																			// 977
						return 1; // L: 978
					} else if (var0 == ScriptOpcodes.CC_SETOPKEYIGNOREHELD) { // L:
																				// 980
						--class446.Interpreter_intStackSize; // L: 981
						var11 = Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize] - 1; // L:
																											// 982
						if (var11 >= 0 && var11 <= 9) { // L: 983
							class145.Widget_setKeyIgnoreHeld(var4, var11); // L:
																			// 986
							return 1; // L: 987
						} else {
							throw new RuntimeException(); // L: 984
						}
					} else if (var0 == ScriptOpcodes.CC_SETOPTKEYIGNOREHELD) { // L:
																				// 989
						var5 = 10; // L: 990
					...
					if (var3) { // L: 927
						...
						for (var7 = 0; var7 < 10 && Interpreter.Interpreter_intStack[var7 + class446.Interpreter_intStackSize] >= 0; var7 += 2) { // L:
																																					// 930
																																					// 931
						}
						if (var7 > 0) { // L: 933
							...
							for (var7 -= 2; var7 >= 0; var7 -= 2) { // L: 936
								var10[var7 / 2] = (byte) Interpreter.Interpreter_intStack[var7 + class446.Interpreter_intStackSize]; // L:
																																		// 937
								var8[var7 / 2] = (byte) Interpreter.Interpreter_intStack[var7 + class446.Interpreter_intStackSize + 1]; // L:
																																		// 938
							}
						}
					} else {
						class446.Interpreter_intStackSize -= 2; // L: 943
						var10 = new byte[]{(byte) Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize]}; // L:
																														// 944
						var8 = new byte[]{(byte) Interpreter.Interpreter_intStack[class446.Interpreter_intStackSize + 1]}; // L:
						// 945
					}
					var7 = Interpreter.Interpreter_intStack[--class446.Interpreter_intStackSize] - 1; // L:
																										// 947
					if (var7 >= 0 && var7 <= 9) { // L: 948
						class340.Widget_setKey(var4, var7, var10, var8); // L:
																			// 951
						return 1; // L: 952
					} else {
	...
	static void SpriteBuffer_decode(byte[] var0) {
		...
		class426.SpriteBuffer_xOffsets = new int[class456.SpriteBuffer_spriteCount]; // L:
																						// 193
		class142.SpriteBuffer_yOffsets = new int[class456.SpriteBuffer_spriteCount]; // L:
																						// 194
		class359.SpriteBuffer_spriteWidths = new int[class456.SpriteBuffer_spriteCount]; // L:
																							// 195
		class456.SpriteBuffer_spriteHeights = new int[class456.SpriteBuffer_spriteCount]; // L:
																							// 196
		class421.SpriteBuffer_pixels = new byte[class456.SpriteBuffer_spriteCount][]; // L:
																						// 197
		var1.offset = var0.length - 7 - class456.SpriteBuffer_spriteCount * 8; // L:
																				// 198
		ModelData0.SpriteBuffer_spriteWidth = var1.readUnsignedShort(); // L:
																		// 199
		class456.SpriteBuffer_spriteHeight = var1.readUnsignedShort(); // L: 200
		int var2 = (var1.readUnsignedByte() & 255) + 1; // L: 201
		int var3;
		for (var3 = 0; var3 < class456.SpriteBuffer_spriteCount; ++var3) { // L:
																			// 202
		...
		var1.offset = var0.length - 7 - class456.SpriteBuffer_spriteCount * 8 - (var2 - 1) * 3; // L:
																								// 206
		class13.SpriteBuffer_spritePalette = new int[var2]; // L: 207
		for (var3 = 1; var3 < var2; ++var3) { // L: 208
			class13.SpriteBuffer_spritePalette[var3] = var1.readMedium(); // L:
																			// 209
