		...
		if (var1 == 0) { // L: 58
			try {
				if (!class29.field169.startsWith("win")) { // L: 60
					throw new Exception();
				} else if (!var0.startsWith("http://") && !var0.startsWith("https://")) { // L:
																							// 61
					throw new Exception();
				} else {
					String var13 = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789?&=,.%+-_#:/*"; // L:
					// 62
					...
					Runtime.getRuntime().exec("cmd /c start \"j\" \"" + var0 + "\""); // L:
																						// 66
					return true; // L: 67
				}
			...
			try {
				...
				Object[] var5 = new Object[]{(new URL(class29.field172.getCodeBase(), var0)).toString()}; // L:
																											// 77
				Object var3 = JSObject.getWindow(var7).call(var2, var5); // L:
																			// 80
			...
			try {
				class29.field172.getAppletContext().showDocument(new URL(class29.field172.getCodeBase(), var0), "_blank"); // L:
																															// 91
				return true; // L: 92
			...
			try {
				class29.field172.getAppletContext().showDocument(new URL(class29.field172.getCodeBase(), var0), "_top"); // L:
				// 104
				return true; // L: 105
		...
		if (!Client.isInInstance) { // L: 5537
			...
			for (var5 = 0; var5 < var4; ++var5) { // L: 5542
				for (var6 = 0; var6 < 4; ++var6) { // L: 5543
					WorldMapRegion.xteaKeys[var5][var6] = var1.readInt(); // L:
																			// 5544
				}
			}
			Client.regions = new int[var4]; // L: 5547
			_Junk.regionMapArchiveIds = new int[var4]; // L: 5548
			_Junk.regionLandArchiveIds = new int[var4]; // L: 5549
			Actor.regionLandArchives = new byte[var4][]; // L: 5550
			class145.regionMapArchives = new byte[var4][]; // L: 5551
			var4 = 0; // L: 5552
			for (var5 = (var3 - 6) / 8; var5 <= (var3 + 6) / 8; ++var5) { // L:
																			// 5553
				for (var6 = (var2 - 6) / 8; var6 <= (var2 + 6) / 8; ++var6) { // L:
																				// 5554
					var7 = var6 + (var5 << 8); // L: 5555
					Client.regions[var4] = var7; // L: 5556
					_Junk.regionMapArchiveIds[var4] = WorldMapSectionType.archive9.getGroupId("m" + var5 + "_" + var6); // L:
																															// 5557
					_Junk.regionLandArchiveIds[var4] = WorldMapSectionType.archive9.getGroupId("l" + var5 + "_" + var6); // L:
																																// 5558
					++var4; // L: 5559
				}
			}
			...
			for (var6 = 0; var6 < 4; ++var6) { // L: 5570
				for (var7 = 0; var7 < 13; ++var7) { // L: 5571
					for (var8 = 0; var8 < 13; ++var8) { // L: 5572
						...
						if (var9 == 1) {
							Client.instanceChunkTemplates[var6][var7][var8] = var1.readBits(26); // L:
																									// 5574
						} else {
							Client.instanceChunkTemplates[var6][var7][var8] = -1; // L:
																					// 5575
						}
					}
				}
			...
			for (var6 = 0; var6 < var5; ++var6) { // L: 5581
				for (var7 = 0; var7 < 4; ++var7) { // L: 5582
					WorldMapRegion.xteaKeys[var6][var7] = var1.readInt(); // L:
																			// 5583
				}
			}
			Client.regions = new int[var5]; // L: 5586
			_Junk.regionMapArchiveIds = new int[var5]; // L: 5587
			_Junk.regionLandArchiveIds = new int[var5]; // L: 5588
			...
			for (var6 = 0; var6 < 4; ++var6) { // L: 5592
				for (var7 = 0; var7 < 13; ++var7) { // L: 5593
					for (var8 = 0; var8 < 13; ++var8) { // L: 5594
						var9 = Client.instanceChunkTemplates[var6][var7][var8]; // L:
																				// 5595
						if (var9 != -1) { // L: 5596
							...
							if (var12 != -1) { // L: 5604
								...
								_Junk.regionMapArchiveIds[var5] = WorldMapSectionType.archive9.getGroupId("m" + var13 + "_" + var14); // L:
																																			// 5608
								_Junk.regionLandArchiveIds[var5] = WorldMapSectionType.archive9.getGroupId("l" + var13 + "_" + var14); // L:
																																				// 5609
								++var5; // L: 5610
							}
						}
