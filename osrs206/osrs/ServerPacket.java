...
public class ServerPacket {
	...
	public static Font method5211(AbstractArchive var0, AbstractArchive var1, String var2, String var3) {
		...
		if (!Ignored.method6774(var0, var4, var5)) { // L: 103
			...
			if (var8 == null) { // L: 110
				...
				Font var9 = new Font(var8, class426.SpriteBuffer_xOffsets, class142.SpriteBuffer_yOffsets, class359.SpriteBuffer_spriteWidths, class456.SpriteBuffer_spriteHeights, class13.SpriteBuffer_spritePalette, class421.SpriteBuffer_pixels); // L:
																																																														// 114
	...
	static void method5210(int var0, Coord var1, boolean var2) {
		WorldMapArea var3 = GrandExchangeOfferAgeComparator.getWorldMap().getMapArea(var0); // L:
																							// 4987
		int var4 = class101.localPlayer.plane; // L: 4988
		int var5 = (class101.localPlayer.x >> 7) + class28.baseX; // L: 4989
		int var6 = (class101.localPlayer.y >> 7) + WorldMapLabelSize.baseY; // L:
																			// 4990
		Coord var7 = new Coord(var4, var5, var6); // L: 4991
		GrandExchangeOfferAgeComparator.getWorldMap().method7292(var3, var7, var1, var2); // L:
																							// 4992
	} // L: 4993
	
	static final void drawEntities(int var0, int var1, int var2, int var3) {
		++Client.viewportDrawCount; // L: 4501
		if (class101.localPlayer.x >> 7 == Client.destinationX && class101.localPlayer.y >> 7 == Client.destinationY) { // L:
			// 4503
			Client.destinationX = 0; // L: 4504
		}
		HealthBarUpdate.method2204(); // L: 4507
		if (Client.combatTargetPlayerIndex >= 0 && Client.players[Client.combatTargetPlayerIndex] != null) { // L:
																												// 4509
			ByteArrayPool.addPlayerToScene(Client.players[Client.combatTargetPlayerIndex], false); // L:
																									// 4510
		}
		...
		Static.setViewportShape(var0, var1, var2, var3, true); // L: 4518
		...
		Rasterizer2D.Rasterizer2D_setClip(var0, var1, var0 + var2, var3 + var1); // L:
																					// 4523
		...
		KeyHandler.method301(InterfaceParent.oculusOrbFocalPointX, class181.field1992, class141.oculusOrbFocalPointY, var4, var5, GZipDecompressor.method8314(var4), var3); // L:
																																											// 4529
		int var6;
		int var7;
		int var8;
		if (!Client.isCameraLocked) { // L: 4531
			...
			if (class19.clientPreferences.method2245()) { // L: 4535
				...
				var8 = SceneTilePaint.getTileHeight(class18.cameraX, class228.cameraZ, PacketWriter.Client_plane); // L:
				// 4539
				if (var8 - class16.cameraY < 800 && (Tiles.Tiles_renderFlags[PacketWriter.Client_plane][class18.cameraX >> 7][class228.cameraZ >> 7] & 4) != 0) { // L:
																																									// 4540
		...
		int var11 = _Junk.cameraYaw; // L: 4552
		int var12;
		int var13;
		for (var12 = 0; var12 < 5; ++var12) { // L: 4553
			if (Client.field743[var12]) { // L: 4554
				var13 = (int) (Math.random() * (double) (Client.field762[var12] * 2 + 1) - (double) Client.field762[var12] + Math.sin((double) Client.field537[var12] / 100.0D * (double) Client.field747[var12]) * (double) Client.field745[var12]); // L:
																																																														// 4555
				if (var12 == 0) { // L: 4556
				...
				if (var12 == 3) { // L: 4559
					_Junk.cameraYaw = var13 + _Junk.cameraYaw & 2047;
				}
				if (var12 == 4) { // L: 4560
		...
		int var14;
		if (var12 >= var0 && var12 < var0 + var2 && var13 >= var1 && var13 < var3 + var1) { // L:
																							// 4573
		...
		StaticSound.playPcmPlayers(); // L: 4590
		Rasterizer2D.Rasterizer2D_fillRectangle(var0, var1, var2, var3, 0); // L:
																			// 4591
		StaticSound.playPcmPlayers(); // L: 4592
		var14 = Rasterizer3D.Rasterizer3D_zoom; // L: 4593
		Rasterizer3D.Rasterizer3D_zoom = Client.viewportZoom; // L: 4594
		class356.scene.draw(class18.cameraX, class16.cameraY, class228.cameraZ, ViewportMouse.cameraPitch, _Junk.cameraYaw, var6); // L:
																																		// 4595
		Rasterizer3D.Rasterizer3D_zoom = var14; // L: 4596
		StaticSound.playPcmPlayers(); // L: 4597
		...
		for (var20 = 0; var20 < var18 + Client.npcCount; ++var20) { // L: 4606
			...
			if (var20 < var18) { // L: 4608
				...
				var31 = Client.npcs[Client.npcIndices[var20 - var18]]; // L:
																		// 4620
			}
			class154.drawActor2d((Actor) var31, var20, var0, var1, var2, var3); // L:
																				// 4621
		}
		if (Client.renderSelf && var17 != -1) { // L: 4623
			class154.drawActor2d(class101.localPlayer, var17, var0, var1, var2, var3); // L:
																						// 4624
		}
		if (var32) { // L: 4626
			class154.drawActor2d(Client.players[Client.combatTargetPlayerIndex], var16, var0, var1, var2, var3); // L:
			// 4627
		}
		for (var20 = 0; var20 < Client.overheadTextCount; ++var20) { // L: 4629
			...
			while (var25) {
				...
				for (int var30 = 0; var30 < var20; ++var30) { // L: 4637
					if (var22 + 2 > Client.overheadTextYs[var30] - Client.overheadTextAscents[var30] && var22 - var24 < Client.overheadTextYs[var30] + 2 && var21 - var23 < Client.overheadTextXs[var30] + Client.overheadTextXOffsets[var30] && var23 + var21 > Client.overheadTextXs[var30] - Client.overheadTextXOffsets[var30] && Client.overheadTextYs[var30] - Client.overheadTextAscents[var30] < var22) { // L:
																																																																																																					// 4638
						// 4639
						var22 = Client.overheadTextYs[var30] - Client.overheadTextAscents[var30]; // L:
																									// 4640
						var25 = true; // L: 4641
					}
				}
			...
			Client.viewportTempY = Client.overheadTextYs[var20] = var22; // L:
																			// 4647
			...
			if (Client.chatEffects == 0) { // L: 4649
				...
				if (Client.overheadTextColors[var20] == 9) { // L: 4655
					var28 = 150 - Client.overheadTextCyclesRemaining[var20]; // L:
																				// 4656
					if (var28 < 50) { // L: 4657
				...
				if (Client.overheadTextEffects[var20] == 4) { // L: 4677
					var28 = (150 - Client.overheadTextCyclesRemaining[var20]) * (TileItem.fontBold12.stringWidth(var26) + 100) / 150; // L:
																																		// 4678
					Rasterizer2D.Rasterizer2D_expandClip(var0 + Client.viewportTempX - 50, var1, var0 + Client.viewportTempX + 50, var3 + var1); // L:
																																					// 4679
					TileItem.fontBold12.draw(var26, var0 + Client.viewportTempX + 50 - var28, Client.viewportTempY + var1, var27, 0); // L:
																																		// 4680
					Rasterizer2D.Rasterizer2D_setClip(var0, var1, var0 + var2, var3 + var1); // L:
																								// 4681
				}
				if (Client.overheadTextEffects[var20] == 5) { // L: 4683
					...
					Rasterizer2D.Rasterizer2D_expandClip(var0, Client.viewportTempY + var1 - TileItem.fontBold12.ascent - 1, var0 + var2, Client.viewportTempY + var1 + 5); // L:
																																											// 4688
					TileItem.fontBold12.drawCentered(var26, var0 + Client.viewportTempX, var29 + Client.viewportTempY + var1, var27, 0); // L:
																																			// 4689
					Rasterizer2D.Rasterizer2D_setClip(var0, var1, var0 + var2, var3 + var1); // L:
																								// 4690
				}
			} else {
				TileItem.fontBold12.drawCentered(var26, var0 + Client.viewportTempX, Client.viewportTempY + var1, 16776960, 0); // L:
																																// 4694
			}
		}
		if (Client.hintArrowType == 2) { // L: 4699
			class220.worldToScreen((Client.hintArrowX - class28.baseX << 7) + Client.hintArrowSubX, (Client.hintArrowY - WorldMapLabelSize.baseY << 7) + Client.hintArrowSubY, Client.hintArrowHeight * 2); // L:
																																																			// 4700
			if (Client.viewportTempX > -1 && Client.cycle % 20 < 10) { // L:
																		// 4701
				UserComparator4.headIconHintSprites[0].drawTransBgAt(var0 + Client.viewportTempX - 12, Client.viewportTempY + var1 - 28);
			}
		}
		((TextureProvider) Rasterizer3D.Rasterizer3D_textureLoader).animate(Client.field555); // L:
																								// 4703
		...
		_Junk.cameraYaw = var11; // L: 4709
		if (Client.isLoading && UrlRequest.method2557(true, false) == 0) { // L:
																			// 4710
	...
	static final void method5209(int var0, int var1) {
		if (Client.currentClanChannels[var0] != null) { // L: 12263
			if (var1 >= 0 && var1 < Client.currentClanChannels[var0].method3113()) { // L:
																						// 12264
				ClanChannelMember var2 = (ClanChannelMember) Client.currentClanChannels[var0].members.get(var1); // L:
				// 12265
				if (var2.rank == -1) {
					PacketBufferNode var3 = EnumComposition.getPacketBufferNode(ClientPacket.field3000, Client.packetWriter.isaacCipher); // L:
																																			// 12267
					var3.packetBuffer.writeByte(3 + ScriptEvent.stringCp1252NullTerminatedByteSize(var2.username.getName())); // L:
					// 12268
					var3.packetBuffer.writeByte(var0); // L: 12269
					var3.packetBuffer.writeShort(var1); // L: 12270
					var3.packetBuffer.writeStringCp1252NullTerminated(var2.username.getName()); // L:
																								// 12271
