...
public abstract class AbstractUserComparator implements Comparator {
	...
	final void addComparator(Comparator var1) {
		...
		} else if (this.nextComparator instanceof AbstractUserComparator) { // L:
																			// 14
			((AbstractUserComparator) this.nextComparator).addComparator(var1); // L:
																				// 15
		}
	} // L: 17
	
	protected final int compareUser(User var1, User var2) {
		return this.nextComparator == null ? 0 : this.nextComparator.compare(var1, var2); // L:
																							// 20
																							// 21
	}
	...
	static final void updatePlayer(PacketBuffer var0) {
		...
		Player var2 = class101.localPlayer = Client.players[var1] = new Player(); // L:
																					// 39
		...
		for (int var7 = 1; var7 < 2048; ++var7) { // L: 55
			if (var1 != var7) { // L: 56
				...
				Players.Players_regions[var7] = (var10 << 14) + var11 + (var9 << 28); // L:
																						// 61
				Players.Players_orientations[var7] = 0; // L: 62
				Players.Players_targetIndices[var7] = -1; // L: 63
				Players.Players_emptyIndices[++Players.Players_emptyIdxCount - 1] = var7; // L:
																							// 64
