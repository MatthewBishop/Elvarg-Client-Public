...
public class WorldMapAreaData extends WorldMapArea {
	...
	void initIconsList(Buffer var1, boolean var2) {
		...
		for (int var4 = 0; var4 < var3; ++var4) {
			...
			if (var2 || !var7) {
				this.iconList.add(new WorldMapIcon_0((Coord) null, var6, var5, (WorldMapLabel) null)); // L:
																										// 53
			}
		}
	}
	...
	static final int method5082(long var0, String var2) {
		...
		int[] var6 = new int[]{var3.nextInt(), var3.nextInt(), (int) (var0 >> 32), (int) var0}; // L:
																								// 38
																								// 39
																								// 40
																								// 41
		...
		if (Client.randomDatData != null) { // L: 62
			var5.writeBytes(Client.randomDatData, 0, Client.randomDatData.length); // L:
																					// 63
		} else {
		...
		Buffer var9 = new Buffer(var4.offset + var5.offset + var8.offset + 5); // L:
																				// 77
		...
		for (int var15 = 0; var15 < var13 + 0; var15 += 3) { // L: 92
			...
			if (var15 < var13 - 1) { // L: 95
				...
				var14.append(class341.field4146[(var16 & 3) << 4 | var17 >>> 4]); // L:
																					// 97
				if (var15 < var13 - 2) { // L: 98
					int var18 = var11[var15 + 2] & 255; // L: 99
					var14.append(class341.field4146[(var17 & 15) << 2 | var18 >>> 6]).append(class341.field4146[var18 & 63]); // L:
																																// 100
				} else {
					var14.append(class341.field4146[(var17 & 15) << 2]).append("="); // L:
																						// 102
				}
			} else {
				var14.append(class341.field4146[(var16 & 3) << 4]).append("=="); // L:
																					// 104
			}
		}
		...
		var12 = var12; // L: 110
		try {
			URL var21 = new URL(ModeWhere.method6080("services", false) + "m=accountappeal/login.ws"); // L:
																										// 112
			...
			OutputStreamWriter var22 = new OutputStreamWriter(var27.getOutputStream()); // L:
																						// 117
			var22.write("data2=" + GrandExchangeOfferOwnWorldComparator.method1097(var12) + "&dest=" + GrandExchangeOfferOwnWorldComparator.method1097("passwordchoice.ws")); // L:
																																												// 118
			var22.flush(); // L: 119
			InputStream var23 = var27.getInputStream(); // L: 120
			var9 = new Buffer(new byte[1000]); // L: 121
			do {
				var17 = var23.read(var9.array, var9.offset, 1000 - var9.offset); // L:
																					// 123
				if (var17 == -1) { // L: 124
					var22.close(); // L: 130
					var23.close(); // L: 131
					...
					if (var24.startsWith("OFFLINE")) { // L: 133
						...
						return 3; // L: 140
					} else if (var24.startsWith("Not permitted for social network accounts.")) { // L:
																									// 142
						return 6; // L: 143
					} else {
						var9.xteaDecryptAll(var6); // L: 145
						while (var9.offset > 0 && var9.array[var9.offset - 1] == 0) { // L:
																						// 146
							--var9.offset; // L: 147
						}
						var24 = new String(var9.array, 0, var9.offset); // L:
																		// 149
						boolean var28;
						if (var24 == null) { // L: 152
							var28 = false; // L: 153
							...
							label107 : {
								try {
									new URL(var24);
								} catch (MalformedURLException var25) { // L:
																		// 159
									var28 = false; // L: 160
